<template>
  <div>
    <div
      :class="`h-10 text-[var(--color-secondary-label)] dark:text-[var(--dark-color-secondary-label)] text-[1rem] rounded-lg flex items-center px-3 font-normal ${
        custom
          ? ''
    ...
    >
      <span>{{ name }}</span>
      <div
        class="text-[var(--color-quaternary-label)] dark:text-[var(--dark-color-quaternary-label)] ml-auto"
      >
        <Icon
          v-if="icon"
          :icon="icon"
